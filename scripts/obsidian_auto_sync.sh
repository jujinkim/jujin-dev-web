#!/bin/bash
################################################################################
# obsidian_auto_sync.sh - Automated Obsidian → Git publish workflow
#
# USAGE:
...
#   3. Waits for initialization (30s) if started, or brief grace (30s) if existing
...
#   - Source vault path: "$HOME/Obsidian Vault/jujin.dev-publish" (contains spaces!)
#
# WARNING:
#   Uses rsync --delete which removes files in target not present in source!
#   Test manually before enabling cron automation.
################################################################################
...
if [[ -z "$(git status --porcelain)" ]]; then
    ...
    if ! git commit -m "Publish update from Obsidian vault

Automated sync at $(date -Iseconds)

🤖 Generated with obsidian_auto_sync.sh"; then
